...
export function testContractAddresses() {
  ...
  expectedTokens.forEach((token) => {
    const address =
      sepoliaAddresses.tokens[token as keyof typeof sepoliaAddresses.tokens];
    if (address) {
...
export function testABIImports() {
  ...
  try {
    ...
    console.log(
      "NigerianStockTokenABI functions:",
      NigerianStockTokenABI.filter(
        (item: { type: string }) => item.type === "function",
      ).length,
    );
    console.log(
      "NGNStablecoinABI functions:",
      NGNStablecoinABI.filter(
        (item: { type: string }) => item.type === "function",
      ).length,
    );
...
export function runAllTests() {
  ...
  if (contractTest && abiTest) {
    console.log(
      "🎉 All tests passed! Token balance fetching should work correctly.",
    );
  } else {
